        ...
        @if (isset($pimpinan) && $pimpinan->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                ...
                @foreach ($pimpinan as $pj)
                    <div
                        class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-md transition-shadow reveal active">

                        <h2
                            class="text-xl md:text-2xl font-bold text-center text-[#2A6F97] mb-6 md:mb-8 pb-4 border-b border-gray-100">
                        ...
                        <div class="grid grid-cols-2 gap-x-4 md:gap-x-12 gap-y-4 justify-items-center">
                            ...
                            <div class="text-center flex flex-col items-center">
                                <div
                                    class="w-24 md:w-32 h-32 md:h-40 mx-auto mb-3 md:mb-4 overflow-hidden rounded-lg shadow-sm border-2 border-white ring-2 ring-gray-100">
                                    @if ($pj->ketua)
                                        <img src="{{ asset('assets/admin/uploads/tenaga_pengajar/' . ($pj->ketua->urlFotoTP ?: 'default.png')) }}"
                                            alt="{{ $pj->ketua->namaTP }}" class="w-full h-full object-cover">
                                    @else
                                        <img src="{{ asset('assets/admin/uploads/tenaga_pengajar/default.png') }}"
                                            alt="Default" class="w-full h-full object-cover">
                                    @endif
                                </div>
                                @if ($pj->ketua)
                                    <p class="text-sm md:text-lg font-bold text-gray-800 mb-0.5">
                                        {{ $pj->ketua->namaTP }}</p>
                                    <p class="text-[10px] md:text-xs font-bold text-[#F2A541]">Ketua Jurusan</p>
                                    {{-- PERBAIKAN: Hapus Carbon, langsung panggil variabelnya --}}
                                    <p class="text-[9px] md:text-[11px] text-gray-500 mt-1">{{ $pj->tahunMulaiPJ }} -
                                        {{ $pj->tahunSelesaiPJ }}</p>
                                @else
                                ...
                                <div
                                    ...
                                    @if ($pj->sekretaris)
                                        <img src="{{ asset('assets/admin/uploads/tenaga_pengajar/' . ($pj->sekretaris->urlFotoTP ?: 'default.png')) }}"
                                            alt="{{ $pj->sekretaris->namaTP }}" class="w-full h-full object-cover">
                                    @else
                                        <img src="{{ asset('assets/admin/uploads/tenaga_pengajar/default.png') }}"
                                            alt="Default" class="w-full h-full object-cover">
                                    @endif
